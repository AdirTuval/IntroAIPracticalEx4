...
class QLearningAgent(ReinforcementAgent):
    ...
    def getPolicy(self, state):
        ...
        if legalActions:
            ...
            action = random.choice([action for action in self.getLegalActions(state)
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.Q_values[(state, action)] = self.Q_values[(state, action)] + (self.alpha * (reward + (self.discount *
                                                                                                   max([self.Q_values[
                                                                                                            (nextState,
                                                                                                             a)]
                                                                                                        for a in
                                                                                                        self.getLegalActions(
                                                                                                            nextState)],
                                                                                                       default=0)) -
                                                                                         self.Q_values[
                                                                                             (state, action)]))
